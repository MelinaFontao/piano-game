...
export default function PianoGame() {
  ...
  const headerSize  = isLandscape && !isTablet ? 22 : 29;
  const showSubtitle = !(isLandscape && !isTablet);
  const badgeFontSz = isTablet ? 18 : 16;
  ...
  return (
    <div style={{
      ...
      <div style={{textAlign:"center",marginBottom:gap}}>
        ...
        {showSubtitle && <div style={{fontSize:14,letterSpacing:5,color:"#6a5030",marginTop:1}}>LECTURA DE NOTAS</div>}
      </div>
      ...
      <div style={{
        ...
        {isKeySigLevel && currentKeySig && (<>
          ...
          <span style={{color:"#4a3020",fontSize:14}}>({currentKeySig.display})</span>
          <span style={{color:"#3a2810"}}>|</span>
          <span style={{color:"#5a4020",fontSize:14}}>{keySigIdx+1}/15</span>
        </>)}
        <span style={{color:"#3a2810"}}>|</span>
        <span style={{color:"#7a6040"}}>
          {subLevel===0?"♾":"⏱ "+SUB_TIMES[subLevel]+"s"}
          {" "}<span style={{color:"#4a3020",fontSize:14}}>({subLevel+1}/{SUB_TIMES.length})</span>
        </span>
      </div>

      {/* Score */}
      <div style={{display:"flex",gap:16,marginBottom:isMobilePortrait?4:gap,fontSize:isTablet?19:17}}>
        ...
        <div style={{width:"100%",maxWidth:isTablet?520:390,marginBottom:isMobilePortrait?6:12,overflowY:"auto"}}>
            ...
            {LEVELS.map((lv,i)=>{
              ...
              return (
                <div key={lv.id} style={{
                  ...
                  <div>
                    <div style={{fontSize:isTablet?18:16,color:isActive?"#f5c842":isDone?"#60a050":"#7a6040"}}>
                      {isDone?"✓ ":isActive?"▶ ":"🔒 "}{lv.id}. {lv.name}
                    </div>
                    <div style={{fontSize:14,color:"#4a3020",marginTop:1}}>{lv.description}</div>
                  </div>
                  {isActive && (
                    <div style={{fontSize:14,color:"#5a4020",textAlign:"right",marginLeft:6}}>
          ...
          <div style={{display:"flex",gap:isMobilePortrait?5:10,justifyContent:"center",flexWrap:"wrap",marginBottom:isMobilePortrait?5:10}}>
            <button onClick={startPlaying} style={{
              ...
              color:"#0e0804",fontSize:isTablet?20:18,fontWeight:"bold",
            ...
            <button onClick={()=>setScreen("stats")} style={{
              ...
              color:"#7a6040",fontSize:isTablet?19:17,cursor:"pointer",fontFamily:"Georgia,serif",
            }}>📊 Stats</button>
            <button onClick={()=>{ if(window.confirm("¿Reiniciar todo el progreso?")) resetGame(); }} style={{
              padding:isTablet?"12px 20px":"10px 16px",borderRadius:28,
              border:"1.5px solid #3a2810",background:"transparent",
              color:"#5a4030",fontSize:isTablet?18:16,cursor:"pointer",fontFamily:"Georgia,serif",
            }}>↺ Reiniciar</button>
          </div>
          ...
          {subLevel > 0 && (
            <div style={{textAlign:"center",marginBottom:10}}>
              <button onClick={continuePlaying} style={{
                ...
                color:"#80c070",fontSize:isTablet?18:16,cursor:"pointer",fontFamily:"Georgia,serif",
              }}>
          ...
          <div style={{textAlign:"center"}}>
            {canLevelUp && levelIdx<LEVELS.length-1 ? (
              <button onClick={handleLevelUp} style={{
                ...
                background:"#50b85018",color:"#70d870",fontSize:17,cursor:"pointer",fontFamily:"Georgia,serif",
              ...
              <div>
                <button onClick={()=>setLevelUpBlocked(true)} style={{
                  ...
                  background:"transparent",color:"#3a2810",fontSize:17,cursor:"not-allowed",fontFamily:"Georgia,serif",
                }}>🔒 Subir de nivel</button>
                {levelUpBlocked && (
                  <div style={{fontSize:15,color:"#a07040",marginTop:5,maxWidth:280,margin:"5px auto 0"}}>
              ...
              <div style={{textAlign:"center"}}>
                <div style={{fontSize:17,color:"#f5c842",marginBottom:8}}>🏆 ¡Nivel máximo alcanzado!</div>
                <button onClick={()=>{ if(window.confirm("¿Reiniciar todo el progreso?")) resetGame(); }} style={{
                  padding:"7px 20px",borderRadius:24,border:"1.5px solid #6a3020",
                  background:"transparent",color:"#a06040",fontSize:16,cursor:"pointer",fontFamily:"Georgia,serif",
